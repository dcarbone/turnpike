...
type Router interface {
	...
	AddSessionOpenCallback(SessionCallbackFunc)
	AddSessionCloseCallback(SessionCallbackFunc)
}
...
type defaultRouter struct {
	...
	sessionOpenCallbacks     []SessionCallbackFunc
	sessionOpenCallbacksLock sync.RWMutex

	sessionCloseCallbacks     []SessionCallbackFunc
...
func (r *defaultRouter) AddSessionOpenCallback(fn SessionCallbackFunc) {
	r.sessionOpenCallbacks = append(r.sessionOpenCallbacks, fn)
}

func (r *defaultRouter) AddSessionCloseCallback(fn SessionCallbackFunc) {
